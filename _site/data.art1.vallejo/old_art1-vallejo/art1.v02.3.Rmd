...
NOTA: sacamos el VCC3,VCC5 y VCC6 y LOY3
...
```{r}
#modelización
modelo.lavaan.out <- matrixpls(cov(select(a.df, c(-10,-12,-13,-22))),modelo.lavaan)
#summary(modelo.lavaan.out) -> no se publica

#Blindfolding -> no se publica
predictions.blindfold <- matrixpls.crossvalidate(cov(select(a.df, c(-10,-12,-13,-22))), 
                                                 model = modelo.lavaan, 
                                                 blindfold = TRUE, 
                                                 predictionType = "redundancy", 
                                                 groups = 7)

prediction_relevance <- q2(cov(select(a.df, c(-10,-12,-13,-22))), predictions.blindfold, model=modelo.lavaan)
